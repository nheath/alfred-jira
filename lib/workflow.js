...
let Icon = (icon, def) => {
  const iconType = color(process.env.alfred_theme_background).grayscale().value() > 0.5 ? 'dark' : 'light';
  const themeIcon = icon.replace(/(.*\/)([^\/]+)$/, `$1${iconType}/$2`);

  if (fs.existsSync(themeIcon)) {
    return themeIcon;
  }
