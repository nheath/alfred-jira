...
process.on('uncaughtException', function (error) {
  ...
  console.log(JSON.stringify({
    items: [{
      ...
      subtitle: 'Check the log for more information',
      valid: false,
      icon: {
        path: 'resources/error.png'
      }
    }]
  }));
...
actions.onAction('mainMenu', async query => {
  query = query.trim();
  ...
  if (Jira.checkConfig()) {
    ...
    await Jira.listAll('bookmark-0').then( tickets => {
      if (tickets) {

        let items = issues.format(tickets)
